...
> ⛔ **Playwright headed 远程演示禁止使用 Test Runner** - 每次演示前必须先通过$DISPLAY变量判断显示设备，远程设备禁止使用 `npx playwright test`，必须用 `node xxx.mjs` 直接运行脚本。Test Runner 会自动关闭浏览器导致连接断裂。
>
> 🛡️ **自动保护**：项目已配置 `playwright.config.ts` 的 `globalSetup`，运行任何 `.spec.ts` 测试时会自动检测并拦截远程显示器，无需记忆。详见 [testing-guide.md](docs/references/testing-guide.md)。
...
- **X11 渲染问题**: `waitForLoadState('networkidle')` 后必须执行 `await page.evaluate(() => window.scrollTo(0, 0))` 触发重绘
